...
class FileSystem {
  ...
  */
  OpenFileId OpenAFile(char *name) {
    ...
    // Find a slot in the open file table for this file.
      for (int i = 0; i <= 20; i++){
  ...
  */
  int WriteFile(char *buffer, int size, OpenFileId id) {
    if (buffer == NULL || size < 0) return -1; //If the buffer is null or the size is less than 0, return -1
    if (id < 0 || id > 20 || OpenFileTable[id] == NULL) return -1; //If the id is less than 0 or greater than 20, or if the OpenFileTable at the id is null, return -1
    int status = OpenFileTable[id]->Write(buffer, size); //If the id is valid, set status to the return value of the Write function, passing in the buffer and size
    if(status < 0) return -1; //If the status is less than 0, return -1
  ...
  */
  int ReadFile(char *buffer, int size, OpenFileId id) {
    if (buffer == NULL || size < 0) return -1; //If the buffer is null or the size is less than 0, return -1
    if (id < 0 || id > 20 || OpenFileTable[id] == NULL)return -1; //If the id is less than 0 or greater than 20 or the OpenFileTable[id] is null, return -1
    int status = OpenFileTable[id]->Read(buffer, size); //Else, read the buffer and the size and assign it to status
    if(status < 0) return -1; //If the status is less than 0, return -1
    return status; //Else, return the status
  }
